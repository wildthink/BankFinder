...
extension Connection {
    ...
    func indentifiers(for table: String, where test: String? = nil) -> [Int] {
        ...
        if let test = test {
            sql = "SELECT id FROM \(table) WHERE \(test)"
        } else {
            sql = "SELECT id FROM \(table)"
        }
        let ids: [Int]? = try? query(sql)
        return ids ?? []
    }

    func count(_ table: String, where test: String? = nil) -> Int {
        var sql: SQL
        if let test = test {
    ...
    func create(view: String, from table: String, select cols: [String]) throws {
    ...
    func create(_ table: String, addID: Bool = true, with defs: String) throws {
    ...
    func format(column: String, as alias: String? = nil) -> String {
    ...
    func insert(into table: String, dictionary: [String:Any]) throws {
        var keys: [String] = ["id"]
    ...
    func json_to_sql(_ value: Any) -> String {
